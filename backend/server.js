...
// Serve static files from client build (after API routes, but exclude index.html for SPA routes)
// Only serve built assets from dist folder - no fallbacks
const distPath = path.resolve(__dirname, "../frontend/dist");
if (!fs.existsSync(distPath)) {
  throw new Error("Frontend build not found. Please build the frontend first: cd frontend && npm run build");
}
...
app.get("*", (req, res, next) => {
  // Skip API routes (plural) and static asset requests
  // Note: /post/ and /product/ (singular) are page routes, not API routes
  const urlPath = req.url.split("?")[0];
  if (urlPath === "/posts" || 
      urlPath === "/products" ||
      urlPath.startsWith("/posts/") ||  // API route: /posts/:id
      urlPath.startsWith("/products/") ||  // API route: /products/:id
      urlPath.startsWith("/api/") ||
      urlPath.startsWith("/assets/") ||
      (urlPath.includes(".") && !urlPath.endsWith(".html"))) {  // Static files but allow .html
    return next();
  }
  
  try {
    // Only use built HTML file from dist folder - no fallbacks
    const htmlPath = path.resolve(__dirname, "../frontend/dist/index.html");
    
    // Check if file exists
    if (!fs.existsSync(htmlPath)) {
      return res.status(500).send("HTML file not found. Please build the frontend first: cd frontend && npm run build");
    }
    ...
    <meta property="og:site_name" content="React SSR SEO App">
    ...
    // Remove any existing og: and twitter: meta tags to avoid duplicates
